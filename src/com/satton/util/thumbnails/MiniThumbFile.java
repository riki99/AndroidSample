...
/**
 * This class handles the mini-thumb file. A mini-thumb file consists of blocks,
 * indexed by id. Each block has BYTES_PER_MINTHUMB bytes in the following
 * format:
 *
 * 1 byte status (0 = empty, 1 = mini-thumb available) 8 bytes magic (a magic
 * number to match what's in the database) 4 bytes data length (LEN) LEN bytes
 * jpeg data (the remaining bytes are unused)
 *
 * @hide This file is shared between MediaStore and MediaProvider and should
 *       remained internal use only.
 */
public class MiniThumbFile {
	...
	/**
	 * We store different types of thumbnails in different files. To remain
	 * backward compatibility, we should hashcode of
	 * content://media/external/images/media remains the same.
	 */
	...
	public static synchronized MiniThumbFile instance(Uri uri) {
		...
		if (file == null) {
			file = new MiniThumbFile(Uri.parse("content://media/external/"
					+ type + "/media"));
	...
	public synchronized long getMagic(long id) {
		...
		if (r != null) {
			...
			try {
				...
				// Other NIO related exception like disk full, read only
				// channel..etc
	...
	public synchronized void saveMiniThumbToFile(byte[] data, long id,
		...
		try {
			...
			// Other NIO related exception like disk full, read only
			// channel..etc
			Log.e(TAG, "couldn't save mini thumbnail data for " + id
					+ "; disk full or mount read-only? " + ex.getClass());
		} finally {
			try {
				if (lock != null)
	...
	public synchronized byte[] getMiniThumbFromFile(long id, byte[] data) {
		...
		try {
			...
			Log.w(TAG, "got exception when reading thumbnail id=" + id
					+ ", exception: " + ex);
		} catch (RuntimeException ex) {
			// Other NIO related exception like disk full, read only
			// channel..etc
